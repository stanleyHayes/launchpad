...
35. ☑ **Creator marketplace commerce and loading states:** organization creators can submit free or one-time paid templates, review their moderation/install metrics, and sell through verified Paystack hosted checkout; paid installation is API-gated by an immutable purchase ledger with a 15% platform fee and seller-earnings balance. Marketplace catalogue pagination plus customer and platform skeleton states prevent empty-state flashes while data loads. — DONE 2026-07-29 (OpenAPI/client contracts, purchase indexes, payment verification and idempotent installation, creator studio UI, marketplace tests, API-client suite, both admin TypeScript checks, and organization production build)
36. ☑ **Employee self-service account settings:** employees can update their display profile and mobile contact, persist notification/digest/language/timezone preferences, change their password with current-password verification and all-session revocation, enroll/confirm/disable TOTP MFA with one-time backup codes, and choose their visual theme from a responsive tabbed settings workspace with skeleton loading. — DONE 2026-07-29 (authenticated account and employee-self contracts, audited service mutations, API-client methods, employee settings/security UI, focused auth/employee tests, API-client suite, and employee production build)
37. ☑ **Manager empty-state contract and Creator Studio geometry:** organization accounts without linked employee profiles receive empty manager rollups instead of raw repository errors, with regression coverage; the marketplace creator panel owns responsive rounded corners and reliably removes inherited surface padding. — DONE 2026-07-29
38. ☑ **Custom dropdown system and marketplace journey builder:** all native frontend selects were replaced with one accessible, portal-rendered LaunchPad listbox that supports forms, controlled filters, keyboard operation, disabled options, and responsive viewport positioning. Creator Studio now builds the actual ordered marketplace journey with multiple typed steps, instructions, deadlines, add/remove, and reordering controls. Legacy feature flags are normalized at the service boundary so zero-value rollout records no longer fail API response validation. — DONE 2026-07-29
39. ☑ **Persistent portal navigation and motion pass:** authenticated sidebar items use hydration-safe client navigation controls so early clicks cannot fall through to document reloads; the shared shell remains mounted while only keyed workspace content receives a fast route transition. Portal navigation/buttons have restrained press and hover feedback, and marketing routes plus below-fold sections receive progressive entrance motion with reduced-motion support. — DONE 2026-07-29
40. ☑ **Platform plan catalog management:** platform billing operators can create and edit subscription plans from the billing workspace, including code, display name, monthly price, currency, description, feature codes, and active availability. Mutations use the existing audited platform plan contracts, refresh the catalog in place, and reject negative pricing at the service boundary. — DONE 2026-07-29
41. ☑ **Constrained plan feature selection and editor action repair:** plan creation and editing use a predefined capability picker with visible selected chips and one-click add/remove behavior instead of free-text feature codes. Toggle geometry is fixed at the shared component level, while plan status and non-wrapping actions occupy a dedicated responsive row. — DONE 2026-07-29
42. ☑ **Account settings parity:** platform staff and organization administrators can edit their personal display name and change their password from clearly labeled Profile and Security settings; password changes retain current-password verification, revoke active sessions, and return the user to login. Employee profile, preference, password, MFA, and appearance controls remain available in the same settings pattern. — DONE 2026-07-29 (all three portal TypeScript checks and diff validation pass)
43. ☑ **Render/Vercel deployment readiness:** Docker-based free-tier Render Blueprint for the API plus Key Value sessions, external MongoDB secret boundary, generated auth/encryption secrets, complete optional-provider placeholders, health checks, build filters, and a lean Docker context; four app-local Vercel project configs; exact and narrowly patterned CORS support for named Vercel previews; and an end-to-end deployment/runbook with environment, domain, and smoke-test guidance. — DONE 2026-07-29
