...
export default function Page() {
  const [status, setStatus] = useState<"idle" | "pending" | "sent">("idle"); const [error, setError] = useState("");
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setStatus("pending"); setError("");
    try { await getClient().requestPasswordReset(String(new FormData(event.currentTarget).get("email") ?? "").trim()); setStatus("sent"); }
    catch (err) { setError(err instanceof ApiError ? err.message : "Unable to request a reset."); setStatus("idle"); }
  }
  return <AuthShell headline="Reset privileged access securely." support="We will send a single-use link that expires automatically.">
